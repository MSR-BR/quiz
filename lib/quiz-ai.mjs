...
function buildSystemPrompt(difficulty) {
  ...
  return [
    ...
    "Inclua também options com exatamente 5 alternativas curtas: a resposta correta e quatro distrações plausíveis, sem marcar qual é a correta.",
...
function normalizeQuestionItem(item, { difficulty, providerName, theme }) {
  ...
  const options = normalizeAnswerOptions(item?.options, answer);
...
function normalizeAnswerOptions(rawOptions, answer) {
  const options = [];

  for (const option of [answer, ...(Array.isArray(rawOptions) ? rawOptions : []), ...getGenericDistractors(answer)]) {
...
function getGenericDistractors(answer) {
  return [
    ...
    "1969",
    "1889",
  ].filter((item) => normalizeLooseText(item) !== normalizeLooseText(answer));
}
