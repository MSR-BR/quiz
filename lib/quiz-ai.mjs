                ...
                items: {
                  ...
                  properties: {
                    ...
                    options: {
                      ...
                      description: "Cinco alternativas curtas. Deve incluir a resposta correta e quatro alternativas plausíveis.",
                    },
                    explanation: {
                      type: "string",
                  ...
                  required: ["question", "answer", "options", "explanation", "hint", "theme", "difficulty"],
                },
              },
            },
            required: ["questions"],
...
function buildSystemPrompt(difficulty) {
  ...
  return [
    ...
    "Inclua também options com exatamente 5 alternativas curtas: a resposta correta e quatro distrações plausíveis do mesmo tema da pergunta, sem marcar qual é a correta.",
    "As alternativas não podem sair do tema solicitado; por exemplo, em Matemática use números, conceitos ou figuras matemáticas, não países ou cidades.",
    "O campo theme de cada item deve repetir exatamente o tema solicitado pelo usuário.",
    ...
    "Retorne, para cada item, exatamente estas chaves: question, answer, options, explanation, hint, theme, difficulty.",
  ].join(" ");
}
...
function normalizeQuestionItem(item, { difficulty, providerName, theme }) {
  ...
  const options = normalizeAnswerOptions(item?.options, answer, theme);
  const explanation = sanitizeText(item?.explanation, 220);
  const hint = sanitizeText(item?.hint, 140);

  if (question.length < 18 || answer.length < 1 || explanation.length < 24 || options.length < 5) {
    return null;
  }
...
function normalizeAnswerOptions(rawOptions, answer, theme) {
  const options = [];

  for (const option of [answer, ...(Array.isArray(rawOptions) ? rawOptions : []), ...getGenericDistractors(answer, theme)]) {
    const text = sanitizeText(option, 90);
    if (!text) {
      continue;
    }

    if (options.some((existing) => normalizeLooseText(existing) === normalizeLooseText(text))) {
      continue;
    }
