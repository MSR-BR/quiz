import { recordSupportMessage } from "./ops-monitor.mjs";
...
export async function sendSupportMessage({ email, message, subject }, env = process.env) {
  ...
  if (!smtpUser || !smtpPass) {
    console.warn("[support] smtp not configured; message logged locally", {
      email: normalizedEmail,
      subject: normalizedSubject,
      text: normalizedMessage,
      to: supportToEmail,
    });
    recordSupportMessage({ delivered: false });
  ...
  recordSupportMessage({ delivered: true });
